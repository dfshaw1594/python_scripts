...
if len(sys.argv) != 6:
  print "Usage - ./ftp_fuzz.py [Target-IP] [Port Number] [Payload]
 [Interval] [Maximum]"
  ...
  print "to include 100 'A's, 200 'A's, etc...up to the maxium of 1000"
    sys.exit()
...
while i <= max:
  try:
      ...
      s=socket.socket(socket.AF_ANET, socket.SOCK_STREAM)
